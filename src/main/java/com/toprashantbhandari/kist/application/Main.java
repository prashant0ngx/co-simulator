package com.toprashantbhandari.kist.application;

import com.toprashantbhandari.kist.assembly.Compiler;
import com.toprashantbhandari.kist.assembly.Program;
import com.toprashantbhandari.kist.simulation.Computer;
import com.toprashantbhandari.kist.simulation.Memory;
import com.toprashantbhandari.kist.simulation.Microoperation;
import com.toprashantbhandari.kist.simulation.RAM;
...
public class Main extends Application {
     ...
     */
    private static final String TITLE = "CO Simulator - KIST";

    /**
     * Text displayed in the 'About' dialog.
     */
    private static final String ABOUT = "Prashant Bhandari \n" +
        "bprashant.kist@gmail.com\n" +
        "19/08/2022\n" +
        "CO Simulator is an assembler program that will input programs \n" +
     ...
     */
    @FXML private TableView<Memory> hexFX;
    /**
     * The contents of the application window.
     */
    @FXML private TableView<Memory> decFX;
    /**
     * The contents of the application window.
     */
    @FXML private TableView<Memory> binFX;
    /**
     * The contents of the application window.
     */
    private Stage stage;
    ...
    private void updateSimulation(Program program) {
        Platform.runLater(() -> {
            if (program.errors().isEmpty()) {
                ...
                decFX.refresh();
                binFX.refresh();
                tabsFX.getSelectionModel().select(0);
...
"Input may come in the form of a single character\n" +
    ...
    private void bind() {
        ...
        hexFX.itemsProperty().bind(computer.ram().valuesProperty());
        computer.pc().valueProperty().addListener((observableValue, number, t1) -> {
            hexFX.getSelectionModel().select(t1.intValue());
            hexFX.scrollTo(Math.max(0, t1.intValue() - 7));
        });

        decFX.itemsProperty().bind(computer.ram().valuesProperty());
        computer.pc().valueProperty().addListener((observableValue, number, t1) -> {
            decFX.getSelectionModel().select(t1.intValue());
            decFX.scrollTo(Math.max(0, t1.intValue() - 7));
        });


        binFX.itemsProperty().bind(computer.ram().valuesProperty());
        computer.pc().valueProperty().addListener((observableValue, number, t1) -> {
            binFX.getSelectionModel().select(t1.intValue());
            binFX.scrollTo(Math.max(0, t1.intValue() - 7));
        });
